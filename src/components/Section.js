...
export default function Section(prop) {
    ...
    const [value, setValue] = useState({foodId: "", foodCalories: "", foodName: "", foodSteps: [], foodIngredients: [], customerId: ""});
        ...
        if (!isIn) {
            ...
            if (null === window.sessionStorage.getItem("userID")) {
                ...
                fetch(endPoint3, {
                    ...
                    body: JSON.stringify({foodId: value.foodId, foodName: value.foodName, provider: "Customer", customerId: window.sessionStorage.getItem("userID")})
                }).then(response=>response.json())
                .then(data => {
